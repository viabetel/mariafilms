...
export function WorkflowSection() {
  ...
  const [timecodes, setTimecodes] = useState(['00:01:14:08', '00:02:45:18', '00:04:12:03']);
  ...
  useEffect(() => {
    const times = initialTimes.current;
  ...
  // Handle color wheel mouse dragging
  ...
  // RequestAnimationFrame animation loop for oscilloscope/VU meters
  useEffect(() => {
    let frameId: number;
    let phase = 0;
    ...
    const tick = () => {
      ...
      setWaveforms({
        r: generateParadePath(0, colorHue),
        g: generateParadePath(2, colorHue + 120),
        b: generateParadePath(4, colorHue + 240)
      });
  ...
  }, [activeIndex, colorHue, colorSat, hookIntensity]);
  ...
  const scrollToStep = (idx: number) => {
    ...
    window.scrollTo({
      top: window.scrollY + rect.top + targets[idx] * totalScrollable,
      behavior: 'smooth'
    });
  ...
  // --- MOBILE LAYOUT (Normal Vertical Flow) ---
  if (isStickyDisabled) {
    return (
      <section 
        id="services" 
        className="relative bg-[#070708] text-white py-16 px-6 border-t border-white/5 overflow-hidden"
      >
        ...
        <div className="container mx-auto flex flex-col gap-10 w-full relative z-10">
          {/* Section Header */}
          ...
          {/* Cards List Stack (Simple Vertical Flow) */}
          <div className="flex flex-col gap-6 relative pl-6 border-l border-white/10 text-left">
            {/* Glowing active indicator line */}
            <div 
              className="absolute left-[-1px] top-0 bottom-0 w-[2px] bg-gradient-to-b from-[#ff007f] to-transparent opacity-60 shadow-[0_0_10px_#ff007f]"
              style={{ height: '70%' }}
            />

            {steps.map((step, idx) => (
              <div key={step.num} className="relative flex flex-col gap-3 items-start group">
                {/* Timeline node */}
                <div className="absolute -left-[31px] top-2.5 w-2 h-2 rounded-full bg-[#ff007f] border border-[#ff007f] shadow-[0_0_6px_#ff007f]" />
                
                {/* Info */}
                <div className="flex flex-col text-left">
                  <span className="text-[9px] tracking-wider text-[#ff007f] font-display-tech font-semibold uppercase">
                    {step.phase} // {step.num}
                  </span>
                  <h3 className="font-display-tech text-white text-lg font-semibold tracking-tight leading-snug lowercase">
                    {step.title}
                  </h3>
                </div>

                {/* Card */}
                <div className="w-full p-5 rounded-xl border border-white/10 bg-[#0a0a0c]/90 relative overflow-hidden">
                  {/* Viewfinder corner crop marks */}
                  <div className="absolute top-3 left-3 border-t border-l border-white/10 w-2 h-2" />
                  <div className="absolute top-3 right-3 border-t border-r border-white/10 w-2 h-2" />
                  <div className="absolute bottom-3 left-3 border-b border-l border-white/10 w-2 h-2" />
                  <div className="absolute bottom-3 right-3 border-b border-r border-white/10 w-2 h-2" />
                  
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-[8px] font-display-tech text-white/30 tracking-widest">
                      [ STAGE_{step.num} // ACTIVE ]
                    </span>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {step.specs.map((spec) => (
                      <span 
                        key={spec} 
                        className="text-[8px] font-display-tech uppercase tracking-wider px-2.5 py-1 rounded-full border border-white/5 bg-white/[0.02] text-neutral-500 lowercase"
                      >
                        {spec}
                      </span>
                    ))}
  ...
  return (
    <section 
      ref={containerRef} 
      id="services" 
      className="relative bg-[#070708] text-white z-20 border-t border-white/5 overflow-hidden h-[250vh]"
    >
      {/* Background Subtle Geometrical Lines */}
      <div className="absolute inset-0 opacity-[0.05] bg-[linear-gradient(to_right,#333_1px,transparent_1px),linear-gradient(to_bottom,#333_1px,transparent_1px)] bg-[size:6rem_6rem] pointer-events-none" />
            ...
            <div className="w-full max-w-[580px] bg-neutral-900 border-[10px] border-neutral-950 rounded-2xl shadow-2xl relative flex flex-col overflow-hidden">
              ...
              <div className="relative aspect-video w-full bg-neutral-950 overflow-hidden flex items-center justify-center border-b border-white/5 group/viewport">
                ...
                <div className="absolute inset-0 z-20 pointer-events-none opacity-40 select-none text-[8px] font-display-tech">
                  ...
                  <span className="absolute bottom-4 right-10 text-white/50">TC {timecodes[activeIndex]}</span>
                </div>
