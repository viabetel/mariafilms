...
export function CinematicAct() {
  ...
  useGSAP(
    () => {
      ...
      if (heroTitle) {
        heroSplit = new SplitText(heroTitle, { type: 'chars,lines', linesClass: 'mask-line' });
      ...
      chapterEls.forEach((el, idx) => {
        ...
        const split = titleEl
          ? new SplitText(titleEl, { type: 'chars,lines', linesClass: 'mask-line' })
          : null;
      ...
      const step = (dir: 1 | -1, absForce: number) => {
        ...
        if (locked) return true; // já dentro da seção, mas ocupado
        const now = performance.now();
        // ignora a cauda inercial do mesmo gesto (eventos decaindo)
        if (now - lastT < 160 && absForce <= lastAbs) {
          lastT = now;
          lastAbs = absForce;
          return true;
        }
      ...
      const onWheel = (e: WheelEvent) => {
        ...
        const handled = step(dir, Math.abs(e.deltaY));
        if (handled) {
          e.preventDefault();
      ...
      const onTouchMove = (e: TouchEvent) => {
        ...
        const handled = step(dir, Math.abs(dy));
        if (handled) {
          e.preventDefault();
          e.stopImmediatePropagation();
        }
