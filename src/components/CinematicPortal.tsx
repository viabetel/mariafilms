...
export function CinematicPortal() {
  ...
  const getStep4Style = () => {
    ...
    // Transição suave mais rápida (de 0.75 a 0.85) para garantir que fique 100% nítido antes do fim da página
    const opacity = isActive ? Math.min(1, (progress - start) / 0.10) : 0;
    const blur = isActive ? Math.max(0, 15 - ((progress - start) / 0.10) * 15) : 15;
    const translateY = isActive ? Math.max(0, 50 - ((progress - start) / 0.10) * 50) : 50;
  ...
  return (
    <section ref={portalRef} className="relative h-[350vh] bg-black z-25">
