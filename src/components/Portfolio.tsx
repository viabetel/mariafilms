...
/**
 * TRABALHOS — scrollytelling horizontal em camadas.
 *
 * No desktop a seção é pinada e o scroll vertical vira um travelling LATERAL
 * pelos filmes. Cada painel tem profundidade: a foto faz parallax para um lado,
 * o número gigante "fantasma" para o outro, e o texto se revela ao centralizar.
 * No mobile, os painéis empilham com reveal vertical (sem pin).
 */
...
function Panel({ film }: { film: Film }) {
  ...
  return (
    <div className="tw-panel relative flex w-full shrink-0 items-center justify-center px-4 py-12 md:h-full md:w-screen md:px-16 md:py-0">
      ...
      <div className="tw-content relative z-[1] grid w-full max-w-6xl grid-cols-1 items-center gap-8 md:grid-cols-2 md:gap-16">
        {/* mídia */}
        <div
          className="tw-imgwrap group relative aspect-[4/5] overflow-hidden rounded-2xl ring-1 ring-white/10 md:aspect-[3/4]"
        ...
        <div>
          <div className="flex items-baseline gap-4">
            ...
            <span className="font-display-tech text-[10px] uppercase tracking-hud text-neutral-500">{film.category}</span>
          </div>
          <h3 className="mt-3 font-serif-editorial text-4xl italic lowercase leading-[0.95] text-white md:text-7xl">
            {film.title}
          </h3>
          <p className="mt-5 max-w-sm font-display-tech text-sm lowercase leading-relaxed text-neutral-400">{film.desc}</p>
          <div className="mt-6 flex items-center gap-6 font-display-tech text-[10px] uppercase tracking-widest text-neutral-500">
            <span>{film.year}</span>
...
export function Portfolio() {
  ...
  useGSAP(
    () => {
      ...
      // DESKTOP — travelling horizontal pinado
      ...
      // MOBILE — painéis empilhados com reveal vertical
      mm.add('(max-width: 767px)', () => {
        gsap.utils.toArray<HTMLElement>('.tw-panel').forEach((panel) => {
          gsap.fromTo(
            panel.querySelector('.tw-imgwrap'),
            { clipPath: 'inset(100% 0 0 0)' },
            { clipPath: 'inset(0% 0 0 0)', duration: 1, ease: EASE.camera, scrollTrigger: { trigger: panel, start: 'top 82%' } },
          );
          gsap.from(panel.querySelector('.tw-content'), {
            y: 40,
            opacity: 0,
            duration: 0.8,
            ease: EASE.in,
  ...
  return (
    <section id="filmes" ref={sectionRef} className="relative z-10 bg-black">
      <div ref={pinRef} className="relative md:h-screen md:overflow-hidden">
        {/* Cabeçalho — overlay no desktop, em fluxo no mobile */}
        <div className="relative z-20 px-4 pt-28 md:absolute md:left-16 md:top-24 md:pt-0">
        ...
        {/* Trilho de painéis */}
        <div ref={trackRef} className="flex flex-col md:h-full md:flex-row">
          {FILMS.map((film) => (
            <Panel key={film.index} film={film} />
          ))}
        </div>

        {/* HUD (desktop): contador + barra + dica */}
        <div className="absolute bottom-10 left-1/2 z-20 hidden -translate-x-1/2 items-center gap-4 md:flex">
          <span ref={counterRef} className="font-display-tech text-xs tracking-widest text-white">01</span>
          <div className="relative h-[2px] w-32 overflow-hidden rounded-full bg-neutral-800">
        ...
        <div className="absolute bottom-10 right-10 z-20 hidden items-center gap-2 font-display-tech text-[10px] uppercase tracking-hud text-neutral-500 md:flex">
          role para navegar <span className="text-pink">→</span>
        </div>
      </div>
    </section>
  );
